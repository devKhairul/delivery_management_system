 ...
 * Get full user list
 */
 $users = $db->query("SELECT * FROM users WHERE userType != 'Administrator'")->fetchAll();
 ...
 if (isset($_POST['loadData']))
 {
   ...
   if (!empty($user))
   {
    ...
    if (empty($fromDate) AND empty($toDate))
    {
      if ($tStatus == '*' AND $user == '*')
      {
        $allTickets = $db->query("SELECT count(*) FROM tickets")->fetchColumn();
        $allComplete = $db->query("SELECT count(*) FROM tickets WHERE ticketStatus = '3'")->fetchColumn();
        $allInProgress = $db->query("SELECT count(*) FROM tickets WHERE ticketStatus = '2'")->fetchColumn();
        $allNotStarted = $db->query("SELECT count(*) FROM tickets WHERE ticketStatus = '1'")->fetchColumn();
        $allDelayed = $db->query("SELECT count(*) FROM tickets WHERE ticketStatus = '4'")->fetchColumn();
        $allBlocked = $db->query("SELECT count(*) FROM tickets WHERE ticketStatus = '5'")->fetchColumn();
        $allArray = $db->query("SELECT * FROM tickets")->fetchAll();
      }
